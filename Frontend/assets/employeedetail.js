const urlParams = new URLSearchParams(window.location.search);
      ...
      async function fetchFreeItemData() {
        try {
          const response = await fetch(
            `http://localhost:5143/Item/GetFreeItems`
          );
          const result = await response.json();
      ...
      async function returnItem(requisitionId) {
        try {
          const response = await fetch(
            "http://localhost:5143/Requisitoned/Return",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      document.getElementById('deleteEmployee').addEventListener('click', async () => {
        ...
        try {
          const response = await fetch(`http://localhost:5143/Employee/Delete?id=${employeeId}`, {
            method: 'POST',
          });
